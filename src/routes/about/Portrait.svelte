<script lang="ts">
    ...
    import OnLoadTransition from "../OnLoadTransition.svelte";

    let width: number = 0;
    let divHeight: number = 0;
    $: divHeight = (width / 2 + width / 30) / 3;

    $: console.log(divHeight >= 426.66 ? 426.66 : divHeight);
    // TODO: Find out how to fix displacement during animation
</script>

<svelte:window bind:outerWidth={width} />
<OnLoadTransition>
    <div class="container w-full max-h-[426.66px]" style="--container-height: {divHeight}">
        <div
            class="w-full relative mt-12 flex flex-col justify-center items-center content-center"
            in:fly={{ y: -200, duration: 1250 }}
        >
            <!-- FIXME: Small hexagons don't stick to positions -->
            <div class="w-2/5 max-w-xs relative">
                <div class="w-3 h-3 absolute bottom-10 left-1/3 bg-primary hex" />
                <div class="w-5 h-5 absolute bottom-2 left-1/2 bg-primary hex" />
                <div class="w-7 h-7 absolute top-0 right-3/4 bg-primary hex" />
